...
const DonorDashboard = () => {
  ...
  const fetchDonations = async () => {
    try {
      const res = await axios.get(API_URL.MY_DONATIONS, {
        headers: {
  ...
  const handleSubmit = async () => {
    try {
      await axios.post(API_URL.CREATE_DONATION, form, {
        headers: {
          Authorization: "Bearer " + token
        }
      });
  ...
  const stats = [
    { label: 'Total Donations', value: donations.length, icon: Package, color: 'bg-blue-500' },
    { label: 'Items Delivered', value: donations.filter(d => d.status === "accepted").length, icon: CheckCircle, color: 'bg-green-500' },
    { label: 'Pending', value: donations.filter(d => d.status === "pending").length, icon: Clock, color: 'bg-yellow-500' },
    { label: 'Impact Score', value: '95', icon: TrendingUp, color: 'bg-purple-500' },
    { label: 'Notifications', value: '8', icon: Bell, color: 'bg-teal-500' }
  ];

  const notifications = [
    { id: 1, type: 'approval', message: 'Your donation has been updated!', time: 'Just now', unread: true }
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50">
      ...
      <header className="bg-white shadow-md border-b-4 border-emerald-400">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between">
          <h1 className="text-3xl font-bold text-emerald-600">Kindkart</h1>
        ...
        {showDonationForm && (
          <div className="fixed inset-0 flex items-center justify-center bg-black/50">
            <div className="bg-white p-6 rounded-xl">
              ...
              <input name="itemType" placeholder="Item" onChange={handleChange} />
